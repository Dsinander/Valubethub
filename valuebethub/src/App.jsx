...
.team-logo{width:18px;height:18px;object-fit:contain;vertical-align:middle;border-radius:2px}
.team-logo-sm{width:14px;height:14px;object-fit:contain;vertical-align:middle;border-radius:2px}
.team-logo-lg{width:24px;height:24px;object-fit:contain;vertical-align:middle;border-radius:2px}
`;
...
export default function App() {
  ...
  const [slipMode, setSlipMode] = useState("smart"); // "smart" = best quality, "target" = hit specific winnings
  ...
  const handleGenerate = useCallback(() => { setPhase("loading"); setLoadProgress(0); setLoadStage(0); setExpandedCards({}); setSaveStatus(null); const n = parseInt(numSelections); const filteredFixtures = fixtures.filter((_, i) => selectedFixtures.includes(i)); const allowedMarkets = new Set(selectedMarketTypes.flatMap(key => MARKET_CATEGORIES[key]?.markets || []));
  // Smart mode: let the AI pick optimal odds based on risk level
  // Target mode: user specifies desired winnings
  let targetOdds;
  if (slipMode === "smart") {
    const smartMultipliers = { conservative: { 1: 1.5, 2: 2.5, 3: 4, 4: 6, 5: 8, 6: 12, 7: 16, 8: 20 }, balanced: { 1: 2.0, 2: 4, 3: 7, 4: 12, 5: 18, 6: 28, 7: 40, 8: 55 }, aggressive: { 1: 3.0, 2: 6, 3: 12, 4: 25, 5: 45, 6: 80, 7: 140, 8: 220 } };
    targetOdds = smartMultipliers[riskLevel]?.[n] || (n * 3);
  } else {
    targetOdds = (parseFloat(targetWinnings) || 500) / (parseFloat(stake) || 100);
  }
  let step = 0; const interval = setInterval(() => { step++; setLoadProgress(Math.min(100, step * (100 / 20))); setLoadStage(Math.min(Math.floor(step / 4), loadStages.length - 1)); if (step >= 20) { clearInterval(interval); const opps = generateOpportunities(filteredFixtures, allowedMarkets); const result = buildSlip(opps, n, riskLevel, targetOdds, slipMode === "target" ? targetWinnings : String(Math.round(targetOdds * (parseFloat(stake) || 100))), stake); setSlip(result); setPhase("results"); } }, 100); }, [numSelections, riskLevel, targetWinnings, stake, selectedFixtures, selectedMarketTypes, fixtures, slipMode]);
      ...
      <div className="app">
        <div className="inner">
          ...
          <div className="nav-tabs">
            ...
            <button className={`nav-tab ${currentPage === "daily-slips" ? "active" : ""}`} onClick={() => goTo("daily-slips")}>🔥 Daily Slips</button>
          ...
          {currentPage === "daily-slips" && <DailySlipsPage />}
          ...
          {!dataLoading && fixtures.length > 0 && phase === "input" && (<>
            ...
            <div className="card"><div className="card-title">Your Parameters</div>
            <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
              <button onClick={() => setSlipMode("smart")} style={{ flex: 1, padding: "10px 14px", borderRadius: 10, border: slipMode === "smart" ? "1px solid var(--gold-500)" : "1px solid var(--navy-600)", background: slipMode === "smart" ? "rgba(212,175,55,0.12)" : "var(--navy-800)", color: slipMode === "smart" ? "var(--gold-400)" : "var(--text-secondary)", fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>🤖 Smart Mode<div style={{ fontSize: 10, fontWeight: 400, marginTop: 2, opacity: 0.7 }}>AI picks the best slip</div></button>
              <button onClick={() => setSlipMode("target")} style={{ flex: 1, padding: "10px 14px", borderRadius: 10, border: slipMode === "target" ? "1px solid var(--gold-500)" : "1px solid var(--navy-600)", background: slipMode === "target" ? "rgba(212,175,55,0.12)" : "var(--navy-800)", color: slipMode === "target" ? "var(--gold-400)" : "var(--text-secondary)", fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>🎯 Target Mode<div style={{ fontSize: 10, fontWeight: 400, marginTop: 2, opacity: 0.7 }}>Set your desired winnings</div></button>
            </div>
            <div className="form-grid">{slipMode === "target" && <div className="field"><label className="field-label">Target Winnings</label><div className="currency-input"><span className="currency-symbol">€</span><input className="field-input" type="number" value={targetWinnings} onChange={e => setTargetWinnings(e.target.value)} /></div></div>}<div className="field"><label className="field-label">Your Stake</label><div className="currency-input"><span className="currency-symbol">€</span><input className="field-input" type="number" value={stake} onChange={e => setStake(e.target.value)} /></div></div><div className="field"><label className="field-label">Selections (1-{Math.min(8, selectedFixtures.length)})</label><input className="field-input" type="number" min="1" max={Math.min(8, selectedFixtures.length)} value={numSelections} onChange={e => setNumSelections(e.target.value)} /></div><div className="field"><label className="field-label">Risk Level</label><div className="risk-options">{["conservative", "balanced", "aggressive"].map(r => <button key={r} className={`risk-btn ${riskLevel === r ? `active-${r}` : ""}`} onClick={() => setRiskLevel(r)}>{r === "conservative" ? "🛡 Safe" : r === "balanced" ? "⚖️ Balanced" : "🔥 Bold"}</button>)}</div></div></div></div>
          ...
          <div className="site-footer">
            <div className="footer-links">
              ...
              <button className="footer-link" onClick={() => goTo("daily-slips")}>Daily Slips</button>
